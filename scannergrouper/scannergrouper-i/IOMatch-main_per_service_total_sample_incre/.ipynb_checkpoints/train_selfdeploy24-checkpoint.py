    ...
    for epoch_per_week in epoch_per_week_list:
        ...
        for protocol in ['http','tls','dns']:
            ...
            for current_date_index in range(len(date_ranges)):
                ...
                cmd2='python train.py --c config/openset_cv/iomatch/'+dataset_name+'_'+protocol+'_incre.yaml' +' --epoch ' +str(epoch) +' --num_train_iter '+ str(epoch*100) +' --date ' +date +' --batch_size ' + str(batch_size) +' --save_name '+ save_name +' --load_path '+load_path
                print(cmd1,cmd2)
                r1=os.system(cmd1)
                print(r1)
                r2=os.system(cmd2)
