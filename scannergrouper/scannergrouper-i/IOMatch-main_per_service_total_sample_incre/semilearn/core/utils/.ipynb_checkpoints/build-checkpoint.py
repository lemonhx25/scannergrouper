...
def get_dataset(args, algorithm, dataset,date, num_labels, num_classes, data_dir='./data',
    ...
    test_dset = None
    if 'openset' in dataset: #in ["cifar10_openset", "cifar100_openset"]:
        lb_dset, ulb_dset, eval_dset, eval_full_dset = get_cifar_openset(args, algorithm, dataset,date, num_labels,
                                                                         num_classes, data_dir=data_dir)
        print('lb_dset',lb_dset,'lb_dset',len(lb_dset))
        #if eval_open:#if eval_open:
        if 1:
...
def get_data_loader(args,
    ...
    void_dataloader=DataLoader(dset, batch_size=batch_size)
    print('void_dataloader',void_dataloader)
