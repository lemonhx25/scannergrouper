...
def sample_labeled_data(args, data, target, num_labels, num_classes, index=None):
    ...
    '''
    print('num_labels',num_labels,'num_classes',num_classes,'this mode use all samples')
    ...
    for c in range(num_classes):
        ...
        print('samples_per_class',samples_per_class,'idx',idx)
        if(len(idx)>0):
            idx=idx
            #idx = np.random.choice(idx, samples_per_class, True)
        else:
            idx=[]
        print('idx_new',idx)
        lb_idx.extend(idx)
