...
class OpenMatchDataset(BasicDataset):
    ...
    def __getitem__(self, idx):
        ...
        img_w=transforms.ToTensor()(img).float()
        if self.name == 'train_lb':
            return {'idx_lb': idx, 'x_lb': img_w, 'x_lb_w_0': img_w, 'x_lb_w_1': img_w,
                    'y_lb': target}
        elif self.name == 'train_ulb':
            return {'idx_ulb': idx, 'x_ulb_w_0': img_w, 'x_ulb_w_1': img_w, 'y_ulb': target}
        elif self.name == 'train_ulb_selected':
            # Selected for FixMatch training
            return {'x_ulb_w': img_w, 'x_ulb_s': img_w}
        #if self.name == 'train_lb':
...
class OpenMatch(AlgorithmBase):
    ...
    def exclude_dataset(self):
        ...
        select_all=select_all.cpu()#update
